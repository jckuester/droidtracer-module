...
static int cb_trace_app(struct sk_buff *skb_2, struct genl_info *info);
static int cb_untrace_app(struct sk_buff *skb_2, struct genl_info *info);
static int cb_whitelist_interface(struct sk_buff *skb_2, struct genl_info *info);
static int cb_blacklist_interface(struct sk_buff *skb_2, struct genl_info *info);
static int cb_set_lowest_uid_traced(struct sk_buff *skb_2, struct genl_info *info);
...
#endif
	if (err) {
		printk(KERN_ERR "RV; sending message failed: %d\n", err);
		return err;
	}
	return 0;
	
 out:
	printk(KERN_ERR "RV; creating netlink message failed: %d\n", err);
	return err;
}
...
int droidtracer_register_genl_ops(void)
{
	...
	err = genl_register_ops(&droidtracer_family, &ops_blacklist_interface);
	if (err)
		goto err;
	
	err = genl_register_ops(&droidtracer_family, &ops_whitelist_interface);
	if (err)
		goto err;
    
	err = genl_register_ops(&droidtracer_family, &ops_set_droidtracer_uid);
	if (err)
		goto err;

	err = genl_register_ops(&droidtracer_family, &ops_set_lowest_uid_traced);
	if (err)
		goto err;
...
static int cb_set_lowest_uid_traced(struct sk_buff *skb_2,
